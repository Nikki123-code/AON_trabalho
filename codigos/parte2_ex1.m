function tabela_final = parte2_ex1 ()
...
for i=1:4
  ...
  resultado=euler_explicito(f,a,b,alfa,N(i));
  resultados_ee=horzcat(resultados_ee,resultado);

  resultado=Heun_explicito(f,a,b,alfa,N(i));
  resultados_heun=horzcat(resultados_heun,resultado);

  resultado=crank_nicolson(f,a,b,alfa,N(i));
  resultados_cn=horzcat(resultados_cn,resultado);

  resultado=euler_implicito(f,a,b,alfa,N(i));
...
for i=1:4
  erro_abs_ee(i)=abs(resultados_ee{i}(end,end)-y(b));
  erro_abs_heun(i)=abs(resultados_heun{i}(end,end)-y(b));
  erro_abs_cn(i)=abs(resultados_cn{i}(end,end)-y(b));
  erro_abs_ei(i)=abs(resultados_ei{i}(end,end)-y(b));
endfor

%calcular o rácio dos erros
for i=1:3
  convergencia_ee(i+1)=erro_abs_ee(i+1)/erro_abs_ee(i);
  convergencia_heun(i+1)=erro_abs_heun(i+1)/erro_abs_heun(i);
  convergencia_cn(i+1)=erro_abs_cn(i+1)/erro_abs_cn(i);
  convergencia_ei(i+1)=erro_abs_ei(i+1)/erro_abs_ei(i);
endfor
